...
// resolvers
const { tools, target } = require('./resolver');

// extend target and tools contexts with package.json of each
if (fs.existsSync(target.path('package.json'))) {
...
global.context.config = {
  ...tools.packageJson.devConfig,
  ...target.packageJson.devConfig,
};

// logger factory -> logger, // FIXME: replace the name with logger after refactoring
const createLogger = tools.require('./logger');
